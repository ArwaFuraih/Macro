...
final class WeatherViewModel: ObservableObject{
    ...
    private func getWeather (coord: CLLocationCoordinate2D?){
        if let coord = coord {
            ...
            let urlString = API.getURLFor(lat: 24.7954, lon: 46.7078)
            getWeatherInternal(city: city, for: urlString)
    ...
    func getWeatherIconFor(icon: String) -> Image {
        switch icon {
        case "01d":
            return Image(systemName: "sun.max.fill") // "clear sky_ day"
        case "01n":
            return Image(systemName: "moon.fill") //"clear _sky_night"
        case "02d":
            return Image(systemName: "cloud.sun.fill")// "few clouds_day"
        case "02n":
